...
`src/multi_agent_system/reporting/` 提供可复用的报告生成框架：

- **报告类型**: TEST_ANALYSIS, KG_HEALTH, BENCHMARK, EXPERIMENT
...
output = generator.create_test_analysis_report(
    test_results={
        "passed": 8,
        "failed": 2,
        "details": [{"name": "test_1", "passed": True}],
    },
...
# 创建 KG 健康报告
output = generator.create_kg_health_report(
    health_data={
        "node_count": 100,
        "edge_count": 500,
        "health_score": 85,
...
```bash
# 生成测试分析报告
python -m multi_agent_system.cli generate-report \
  --type test_analysis \
  --format markdown \
