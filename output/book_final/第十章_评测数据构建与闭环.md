...
class DataQualityMonitor:
    ...
    def check_completeness(self, data):
        """检查完整性"""
        total = len(data)
        missing = sum(1 for d in data if self._is_missing(d))
        return 1 - missing / total
    
    def check_accuracy(self, data, ground_truth):
        """检查准确性"""
        correct = sum(1 for d, gt in zip(data, ground_truth) if d == gt)
    ...
    def check_timeliness(self, data, update_time):
        """检查时效性"""
        age = time.time() - update_time
        return max(0, 1 - age / (7 * 24 * 3600))  # 7天过期
```
