...
其中$\$$可以使用知识图谱中的语义相似度计算：

$$sim_{KG}(i, j) = \frac{2 \cdot |KG_{path}(i) \cap KG_{path}(j)|}{|KG_{path}(i)| + |KG_{path}(j)|}$$

**推荐理由质量评估**：

$$Quality_{reason} = lpha \cdot Coherence + eta \cdot Persuasiveness + \gamma \cdot Relevance$$
...
对话系统的输出质量难以用简单指标衡量，LLM-as-Judge可以评估对话的各个维度。对话质量评估需要考虑：
...
**支付服务分类评估**：在支付宝小程序场景中，服务类型包括：转账、理财、缴费、贷款、保险等。LLM-as-Judge需要准确评估服务分类的正确性：

$$Accuracy_{service} = \frac{\sum_{i=1}^{N} \mathbb{1}[pred_i = true_i]}{N}$$

**金融合规性评估**是金融场景的核心要求。LLM-as-Judge需要检测：
...
> **实验数据：** 在支付宝服务理解评测中，LLM-as-Judge对金融合规性的判断准确率达到92.3%，与人工审核的一致性达到87.5%。
...
其中$\$$包括：功能匹配、价格匹配、质量匹配、位置匹配、时间匹配等维度。

> **实验数据：** 在支付宝小程序服务匹配评测中，基于知识图谱增强的LLM-as-Judge在NDCG@10上达到0.847，相比传统BM25方法的0.612有显著提升。
...
### 7.4.4 LLM-as-Judge方法的实验对比

不同LLM-as-Judge方法在各评测任务上的表现存在显著差异，下表汇总了主要方法的实验结果：

| 方法 | 评测任务 | 指标 | 性能表现 | 年份 |
|------|----------|------|----------|------|
| GPT-4 as Judge | 摘要生成 | 人类一致性 | 80.4% | 2023 |
| Claude as Judge | 代码生成 | 人类一致性 | 78.2% | 2023 |
| LLaMA-3 as Judge | 对话评测 | 准确率 | 72.5% | 2024 |
| KG增强LLM | 服务匹配 | NDCG@10 | 0.847 vs 0.612 | 2024 |
| 多评判者投票 | 综合评测 | 偏见降低 | -35% | 2024 |
| 长度归一化 | 开放式生成 | 偏见降低 | -40% | 2023 |
| 随机位置交换 | 成对比较 | 位置偏见 | -45% | 2023 |
| Self-Contrast | 模型自评 | 一致性 | +18% | 2024 |

> **实验数据分析：**
> - GPT-4在人类一致性方面表现最佳，但在某些特定领域可能存在知识盲区
> - 知识图谱增强的LLM-as-Judge在专业领域（如支付服务）效果显著
> - 多评判者投票和位置交换可以有效降低评估偏见
...
$$Score_{normalized} = \f\frac{Score}{length^\alpha}$$
