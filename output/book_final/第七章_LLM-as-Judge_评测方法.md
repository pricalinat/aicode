...
$$Diversity@K = rac{1}{K(K-1)}\sum_{i 
eq j} sim(item_i, item_j)$$

其中$\$$可以使用知识图谱中的语义相似度计算：

$$sim_{KG}(i, j) = rac{2 \cdot |KG_{path}(i) \cap KG_{path}(j)|}{|KG_{path}(i)| + |KG_{path}(j)|}$$
...
$$Completeness = rac{|KG_{extracted} \cap KG_{generated}|}{|KG_{extracted}|}$$
...
$$Accuracy_{service} = rac{\sum_{i=1}^{N} \mathbb{1}[pred_i = true_i]}{N}$$
...
$$ComplianceScore = rac{\sum_{d \in D} w_d \cdot Compliance_d}{\sum_{d \in D} w_d}$$
...
$$Discoverability = rac{#\{services\ correctly\ matched\}}{#\{all\ relevant\ pairs\}}$$
