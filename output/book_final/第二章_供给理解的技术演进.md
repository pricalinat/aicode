...
深度学习技术在供给理解领域的应用经历了几个重要阶段：2013-2016年是深度学习在推荐系统中的初步应用阶段；2017-2019年是Transformer和BERT的爆发期；2020年至今是预训练模型的大规模应用和多样化发展阶段。本章将沿着这一技术演进脉络，深入剖析每一代技术的核心原理和应用方法。
...
本章共分为九个主要部分。2.2节介绍深度学习基础，包括神经网络的基本原理、常见的激活函数和优化方法。2.3节重点剖析Transformer架构，这是现代预训练语言模型的核心基础。2.4节深入讲解BERT模型及其变体，涵盖模型架构、预训练任务和微调方法。2.5节介绍GPT系列模型，重点分析自回归语言模型的技术特点。2.6节讨论预训练模型在供给理解中的应用方法，包括商品搜索排序、商品推荐、商品分类等具体任务。2.7节进行原创分析与实验对比，2.8节介绍技术解法分类体系，2.9节对本章进行总结。
...
在电商场景中，RNN及其变体被广泛应用于用户行为序列建模，这些应用展示了循环神经网络在理解用户行为和商品评价方面的能力。

### 2.2.5 正则化与归一化技术

正则化技术是防止神经网络过拟合的重要手段。在供给理解的电商场景中，训练数据往往存在分布不均和长尾问题，正则化技术尤为重要。**Dropout正则化**通过在训练过程中随机"丢弃"部分神经元来防止过拟合，其核心思想是迫使网络学习更加鲁棒的特征表示。在每次前向传播时，每个神经元以概率$p$被暂时关闭，这种方法在商品分类和推荐排序模型中被广泛使用，有效提升了模型的泛化能力。**标签平滑**将硬标签转换为软标签分布，防止模型对训练数据过于自信，在电商商品类目预测中可以帮助模型更好地区分相似类目。**层归一化**与批归一化不同，它在单个样本的特征维度上进行归一化，不依赖于batch size，这种方法被广泛应用于Transformer架构中，是BERT等预训练模型的核心组件。**残差连接**通过引入恒等映射，使得梯度能够直接流向更深的层，有效缓解深层网络的梯度消失问题，这种设计使得构建超过百层的深度网络成为可能，也为后续的大规模预训练模型奠定了基础。

## 2.3 Transformer架构

Transformer架构包含编码器和解码器两个核心部分，其核心是自注意力（Self-Attention）机制。编码器由多个相同的层堆叠而成，每层包含多头自注意力层和位置全连接前馈网络。解码器同样由多个相同的层堆叠而成，包含掩码多头自注意力、编码器-解码器注意力和前馈网络。

### 2.3.1 自注意力机制

自注意力机制能够直接建模序列中任意两个位置之间的依赖关系，不受距离限制。自注意力的计算过程如下：
...
旋转位置编码（Rotary Position Embedding，RoPE）是近年来最重要的位置编码改进之一，被广泛应用于LLaMA等大语言模型中。RoPE的核心思想是将位置信息编码为旋转矩阵，从而在不改变注意力机制的情况下引入相对位置信息。

RoPE的数学表达式为：

$$f_q(x_m, m) = W_q \cdot R_{\Theta,m}^d \cdot x_m$$
$$f_k(x_n, n) = W_k \cdot R_{\Theta,n}^d \cdot x_n$$

其中，$R_{\Theta,m}^d$是旋转矩阵，定义为：
...
RoPE的优势在于：其一，它能够自然地建模相对位置关系；其二，通过旋转矩阵的稀疏结构，RoPE可以在保持效果的同时降低计算开销；其三，RoPE具有良好的外推能力，可以处理超出训练长度的序列。

**ALiBi（Attention with Linear Biases）**

ALiBi是另一种创新的位置编码方法，它通过在注意力分数上添加线性偏置来实现位置信息的引入，而不需要额外的位置嵌入。ALiBi的注意力计算为：

$$Attention(Q, K, V) = softmax\left(\frac{QK^T}{\sqrt{d_k}} + m \cdot \left[-log(n+1)\right]_{i,j}\right)V$$

其中，$m$是可学习的标量，$\left[-log(n+1)\right]_{i,j}$是预先计算的距离矩阵。ALiBi的优势在于它不引入额外的位置嵌入参数，并且具有优秀的泛化能力。

**相对位置编码的其他变体**

T5模型采用了相对位置编码，通过偏置项来编码位置差异：

$$Attention(Q, K, V) = softmax\left(\frac{QK^T}{\sqrt{d_k}} + b_{j-i}\right)V$$

其中，$b_{j-i}$是相对位置偏置。DeBERTa进一步改进了这一方法，采用了解耦的相对位置编码和增强的注意力机制。

这些位置编码方法的演进，反映了研究者对Transformer架构的持续优化，也为构建更大规模、更长上下文的语言模型提供了技术基础。
...
BERT在电商搜索和推荐领域得到了广泛应用。例如，BERT被用于搜索结果的重排序（passage reranking），显著提升了搜索结果的相关性，证明了双向Transformer编码器在理解用户查询和商品描述方面的优势。

Sentence-BERT（SBERT）是BERT的重要扩展，通过引入孪生网络结构，使得BERT能够生成句子级别的语义表示。在电商场景中，SBERT被广泛应用于：

- 商品相似度计算：将商品标题和描述编码为向量，计算商品之间的相似度
- 语义搜索：将用户查询和商品表示映射到同一向量空间，实现语义匹配
- 商品聚类：将相似商品聚类，用于商品组织和发现

研究者提出了BERT2DNN模型，进一步提升了电商搜索的性能。

然而，BERT在工业应用中仍面临挑战：推理延迟较高，单次推理需要50-100毫秒，在高并发场景下成本昂贵；模型参数量大，部署和维护成本高；商品标题通常较短，BERT的双向注意力机制在短文本上的优势难以充分发挥。针对这些挑战，工业界普遍采用知识蒸馏、模型剪枝、量化等技术进行优化。
...
GPT（Generative Pre-training）系列采用自回归（Autoregressive）方式建模语言。与BERT的双向编码器不同，GPT基于Transformer解码器构建，其注意力机制是单向的。
### 2.5.1 自回归语言模型
...
### 2.5.4 大语言模型在电商场景的挑战与应对

尽管以ChatGPT为代表的大语言模型在通用领域展现了强大的能力，但将其应用于电商场景仍面临诸多挑战。

**挑战一：推理成本与实时性要求**
电商场景对响应时间有严格要求，典型搜索系统需要在毫秒级返回结果。然而，大语言模型的推理成本高昂，GPT-3单次推理延迟可达数秒。应对策略包括：模型蒸馏（如DistilBERT、QLora）、量化推理（INT8/INT4量化）、以及异步处理架构。阿里云团队提出的PAI-Easel系统通过推理优化将BERT-base的延迟控制在10ms以内，为大模型在电商场景的应用提供了参考。

**挑战二：领域知识与时效性**
通用大语言模型的训练数据存在知识截止日期，难以准确回答价格变动、新品上市等时效性问题。解决方案包括：检索增强生成（RAG）框架，通过实时检索商品数据库和知识库为模型提供最新信息；微调方法，使用电商领域对话数据对模型进行持续训练。京东研究院提出的E-commerce RAG系统，将商品知识库与大模型相结合，显著提升了问答准确性。

**挑战三：幻觉问题与可信度**
大语言模型可能生成不存在的商品信息或错误的价格描述，这在电商场景中是不可接受的。应对方法包括：约束解码策略，限制输出内容必须来自已知商品库；事实核查机制，对生成的商品属性进行交叉验证；知识图谱增强，利用结构化知识约束生成内容。淘宝团队提出的"商品知识增强对话"方案，通过将商品属性知识注入提示上下文，有效减少了幻觉问题。

**挑战四：多语言与方言理解**
跨境电商需要处理多语言商品描述和用户查询，不同语言间的语义对齐和方言理解仍是难题。Multilingual BERT和XLM-R等跨语言模型提供了基础能力，但在小语种和方言上的效果仍有提升空间。针对中文电商场景，方言理解和网络用语处理也需要专门优化。

**挑战五：评测与业务指标对齐**
学术评测指标（如BLEU、ROUGE）与电商业务指标（点击率、转化率、GMV）之间存在差距。如何建立与业务目标一致的大模型评测体系，是工程落地的重要课题。1688平台实践表明，基于A/B测试的在线评测比离线指标更能反映模型的实际业务价值。


### 2.5.5 大语言模型智能体在电商中的应用

大语言模型智能体（LLM Agent）代表了人工智能系统设计的新范式，它将大语言模型的推理能力与工具使用、规划分解、自我反思等能力相结合，为供给理解带来了新的可能性。与传统的静态模型不同，智能体能够自主决策执行顺序、动态调用外部工具、并在执行过程中持续优化策略。

**智能体架构的核心组件**

典型的电商LLM智能体包含以下核心组件：首先是规划模块（Planning），负责将复杂任务分解为可执行的子任务；其次是记忆模块（Memory），包括短期记忆（当前对话上下文）和长期记忆（历史交互经验）；第三是工具库（Tools），集成了商品搜索API、知识图谱查询、数据库检索等外部能力；最后是执行引擎（Execution），负责按照规划调用工具并收集结果。

**商品对话助手的智能化升级**

传统的商品问答系统通常采用检索-排序-生成的级联架构，而基于LLM智能体的商品对话助手则能够实现更加动态和个性化的交互体验。当用户提出"帮我找一款适合敏感肌的护肤品"这类模糊需求时，智能体会先通过追问明确用户的具体需求（如肤质、预算、品牌偏好等），然后利用商品检索工具获取候选商品，最后根据用户的反馈进行多轮筛选和推荐。这种主动式交互范式显著提升了用户体验和转化率。

**智能体在商品搜索排序中的应用**

传统的搜索排序模型需要大量的人工特征工程，且难以处理长尾查询。LLM智能体通过"推理-检索-重排"的新范式，能够在运行时动态理解用户查询意图。具体而言，智能体首先分析查询的语义和用户潜在意图，然后检索相关的商品候选集，最后结合商品属性和用户偏好进行个性化重排。这种方法在长尾查询和复杂多条件查询场景中表现出显著优势。

**挑战与未来方向**

尽管LLM智能体在电商场景展现出巨大潜力，但其实际部署仍面临诸多挑战。首要挑战是响应延迟，智能体的多轮推理和工具调用增加了整体延迟，需要通过缓存、并行化等手段进行优化。其次是稳定性问题，智能体的自主决策可能导致不一致的行为，需要建立完善的监控和干预机制。第三是可解释性需求，电商场景对推荐理由有明确需求，智能体的决策过程需要提供可追溯的解释依据。

未来，随着多模态大语言模型的发展，智能体将能够直接理解和处理商品图片、视频等视觉内容，实现更加全面的供给理解。同时，个性化智能体将成为趋势，每个用户可能拥有专属的购物助手，能够记忆用户偏好、学习消费习惯，提供真正个性化的购物体验。
...
这些方法都为商品分类提供了重要的技术参考。


## 2.7 原创分析与实验对比

### 2.7.1 原创洞察：深度学习演进的技术洞察
...
这一分类体系帮助读者从技术演进的角度理解供给理解问题的解决路径。

### 2.8.1 基于嵌入的表示学习方法

嵌入方法（Embedding）是将高维稀疏向量映射到低维稠密向量空间的核心技术。

**词嵌入阶段**：Word2Vec和GloVe等技术通过无监督学习捕捉词的语义表示。神经协同过滤模型将嵌入方法引入推荐系统。

**预训练语言模型阶段**：通过掩码语言模型和下一句预测任务学习上下文相关的表示。进一步优化了句子级别的语义匹配。

**大语言模型阶段**：通过指令微调实现了更强的泛化能力，电商领域专用模型进一步针对行业特点进行了优化。

### 2.8.2 基于图结构的方法

图神经网络（GNN）在建模供给之间的复杂关系方面表现出色。

**图嵌入方法**：TransE、RotatE等知识图谱嵌入方法能够学习实体和关系的向量表示。

**图神经网络方法**：GCN、GAT等模型通过消息传递机制聚合邻居信息。
...
**RNN/LSTM方法**：能够有效展示对用户购买序列的建模能力。

**Transformer方法**：BERT4Rec使用双向Transformer对用户行为序列进行建模。

**长序列建模**：解决了超长行为序列的效率问题，在淘宝系统中得到应用。
...
不同技术方法在各任务上的性能表现存在显著差异，下表汇总了主要方法的实验结果：

| 方法 | 任务 | 指标 | 性能 | 年份 |
|------|------|------|------|------|
| TF-IDF | 商品检索 | MAP | 0.45 | 2010 |
| Word2Vec | 语义匹配 | NDCG@10 | 0.52 | 2013 |
| LSTM | 序列建模 | Accuracy | 78.5% | 2016 |
| BERT | 语义匹配 | NDCG@10 | 0.85 | 2019 |
| GPT-2 | 商品生成 | BLEU | 32.1 | 2020 |
| GPT-3 | 零样本分类 | Accuracy | 72.4% | 2021 |
| CLIP | 图文匹配 | Accuracy | 76.2% | 2021 |
| LLM + RAG | 商品问答 | F1 | 89.3% | 2024 |

> **实验数据分析：**
> - BERT在语义匹配任务上相比Word2Vec提升了约63%的NDCG@10性能
> - LLM结合RAG在问答任务上展现出显著优势，能够利用外部知识库
> - CLIP在零样本图像分类上表现突出，无需特定领域训练数据
...
最后，我们展示了预训练模型在商品搜索排序、商品推荐、商品分类等供给理解具体场景中的应用方法。基于这些应用实践，我们可以看到预训练模型在电商搜索、推荐等场景中取得了显著的效果提升。
