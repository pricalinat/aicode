...
在电商场景中，RNN及其变体被广泛应用于用户行为序列建模.，都展示了循环神经网络在理解用户行为和商品评价方面的应用。
...
BERT在电商搜索和推荐领域得到了广泛应用。中的passage reranking，显著提升了搜索结果的相关性.证明了双向Transformer编码器在理解用户查询和商品描述方面的优势。
...
最后，我们展示了预训练模型在商品搜索排序、商品推荐、商品分类等供给理解具体场景中的应用方法。基于，我们可以看到预训练模型在电商搜索、推荐等场景中取得了显著的效果提升。
