...
$$R_{\Theta,m}^d = egin{pmatrix} \cos(m	heta_1) & -\sin(m	heta_1) & 0 & 0 \ \sin(m	heta_1) & \cos(m	heta_1) & 0 & 0 \ 0 & 0 & \cos(m	heta_2) & -\sin(m	heta_2) \ 0 & 0 & \sin(m	heta_2) & \cos(m	heta_2) \end{pmatrix}$$
...
$$Attention(Q, K, V) = softmax\left(rac{QK^T}{\sqrt{d_k}} + m \cdot \left[-log(n+1)
ight]_{i,j}
ight)V$$

其中，$m$是可学习的标量，$\left[-log(n+1)
ight]_{i,j}$是预先计算的距离矩阵。ALiBi的优势在于它不引入额外的位置嵌入参数，并且具有优秀的泛化能力。

**相对位置编码的其他变体**

T5模型采用了相对位置编码，通过偏置项来编码位置差异：

$$Attention(Q, K, V) = softmax\left(rac{QK^T}{\sqrt{d_k}} + b_{j-i}
ight)V$$
...
（SBERT）是BERT的重要扩展，通过引入孪生网络结构，使得BERT能够生成句子级别的语义表示。在电商场景中，SBERT被广泛应用于：
...
**（1）数据偏差与泛化性问题**
多数研究依赖于特定平台的公开数据集（如Amazon、Taobao），这些数据集存在明显的选择偏差。在MovieLens和Pinterest数据集上的实验显示，模型在不同数据分布下性能差异显著。的实验虽然包含真实线上环境，但 Singles Day 等高流量场景的数据代表性仍然有限。
**（2）评估指标与业务目标的对齐问题**
现有研究大多采用HR@K、NDCG等学术指标评估模型效果，但这些指标与实际业务的转化率、GMV等目标存在差距.与实际推荐系统的在线效果之间存在gap。
**（3）可解释性与可解释AI的需求**
深度学习模型的黑箱特性限制了其在电商场景中的应用.当前模型难以提供有说服力的推荐理由。
**（4）计算效率与可扩展性**
预训练模型（如BERT）的计算开销限制了其在实时系统中的大规模部署。在淘宝搜索中的部署面临显著的延迟挑战。
...
**词嵌入阶段**：Word2Vec和GloVe等技术通过无监督学习捕捉词的语义表示.提出了神经协同过滤模型。

**预训练语言模型阶段**：通过掩码语言模型和下一句预测任务学习上下文相关的表示。进一步优化了句子级别的语义匹配。

**大语言模型阶段**：通过指令微调实现了更强的泛化能力，eCeLLM专门针对电商场景进行了优化。
